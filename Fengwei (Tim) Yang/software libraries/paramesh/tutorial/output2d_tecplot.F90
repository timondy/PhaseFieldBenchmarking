...
! 
! This routine captures output from all processors, onto processor 0
! and outputs it in a format suitable for TecPlot. It will create a
! separate file for each refinement level within the grid. It assumes
! you are using the SHMEM library for inter-processor communications.
!
!
...
! get properties of the grid block lb on processor iproc
		call shmem_integer_get(crefine,lrefine(lb),1,iproc)
		call shmem_real_get(csize(1),bsize(1,lb),3,iproc)
		call shmem_real_get(ccoord(1),coord(1,lb),3,iproc)
		call shmem_real_get(cbnd_box(1,1),bnd_box(1,1,lb),6, & 
     &                                                      iproc)


! get cell centered data for the grid block lb on processor iproc
     ...
     &					len_block,iproc)
		...
		do j=nguard0+1,nyb+nguard0
                yi = cbnd_box(1,2)+dy*real(j-nguard0-.5)
                do i=nguard0+1,nxb+nguard0
                xi = cbnd_box(1,1)+dx*real(i-nguard0-.5)

! write the coords for each cell and the first word in unk
		den =  cunk(1,i,j,k)
...
50      format(3(1x,1pe13.6))
	...
	enddo
      ...
      return
      end subroutine output2d_tecplot

...
! copy 0 to the current time.
          ...
          do k=kl_bnd+nguard*k3d,ku_bnd-nguard*k3d+k3d
            write(*,*) 'before reset face k ',k,' block ',lb
             do j=1,nyb+2*nguard
             write(*,58) j,(facevarz(1,i,j,k,lb), & 
     &                                             i=1,nxb+2*nguard)
             enddo
          enddo
          endif
...
! Set solution copies  P = 0 : nphase-1 to be the same as copy P = nphase.
          ...
          if(lb.eq.6) write(*,*) 'resetting soln on blk 6 : ', & 
     &         ivar1,' to ',ivar2,' from ',ivar3,' to ',ivar4
            if(nvar.gt.0) & 
     ...
     &          facevarz(ivar3:ivar4,:,:,:,lb)
          ...
          if(lb.eq.6) then
          do k=kl_bnd+nguard*k3d,ku_bnd-nguard*k3d+k3d
            write(*,*) 'after reset face k ',k,' block ',lb
             do j=1,nyb+2*nguard
             write(*,58) j,(facevarz(1,i,j,k,lb), & 
     &                                             i=1,nxb+2*nguard)
             enddo
...
#ifdef VAR_DT
          if( lcycle.eq.1 ) then
         if(lb.eq.7) write(*,*) 'reset ttfluxes : blk 7 ', & 
...
! Copy data from current block into working block and fill its guardcells
     ...
     &                          lcc,lfc,l_srl_only,icoord,ldiag)
          ...
          if(lb.eq.6) then
          do k=kl_bnd+nguard*k3d,ku_bnd-nguard*k3d+k3d
            write(*,*) 'bef adv face k ',k,' block ',lb
             do j=1,nyb+2*nguard
             write(*,58) j,(facevarz(1,i,j,k,lb), & 
     &                                             i=1,nxb+2*nguard)
             enddo
...
! Capture solution for current phase from temporary storage.
          ...
          if(lb.eq.7) write(*,*) 'capture soln on blk 7 : ', & 
     &        ivar1,' to ',ivar2,' from ',ivar1,' to ',ivar2
           if(nvar.gt.0) & 
...
!     .        xtest.lt.0..and.ytest.lt.0..and.ztest.lt.0.) then
           if(l.eq.6) then
             ...
             do k=1+nguard0*k3d,nzb+1+nguard0*k3d
!             k = nzb+1+nguard0*k3d
             write(*,*) 'k plane ',k
             do j=1,nyb+2*nguard0
             write(*,55) j,(facevarz(ivar1,i,j,k,l),i=1,nxb+2*nguard0)
55           format(1x,i3,6(2x,f7.4))
             enddo
             enddo
           endif
...
! modify flux/variable association for the current timestep phase
     ...
     &                                                       )
          ...
          if(lb.eq.6) then
          do k=kl_bnd+nguard*k3d,ku_bnd-nguard*k3d+k3d
            write(*,*) 'aft eaver face k ',k,' block ',lb,' var 3'
             do j=1,nyb+2*nguard
             write(*,58) j,(facevarz(3,i,j,k,lb), & 
     &                                             i=1,nxb+2*nguard)
58           format(1x,i3,6(2x,f7.4))
             enddo
          enddo
...
!             write(*,50) j,(unk(2,i,j,k,lb),i=1,nxb+2*nguard0)
      ...
      lcycle = loc_cycle(ilev)
      ivar = nphase*nvarp + 1
      write(*,*) 'Output for 7 ? : lcycle ncyc_local ', & 
     &                  lcycle, ncyc_local(ilev)
...
! set values for the solution at the end of timestep stage PHASE0
     ...
     &        - (ez_x(ivar,i,j,k) - ez_x(ivar,i,j+k2d,k))*dx )
            ...
            if(iblock.eq.6) then
            write(*,*) 'face k ',k,' ivar+nvarp ',ivar+nvarp, & 
     &                   ' iv ',iv
             do j=1,nyb+2*nguard
             write(*,56) j,(facevarz1(ivar+nvarp,i,j,k,idest), & 
     &                                             i=1,nxb+2*nguard)
             write(*,56) j,(facevarz1(iv,i,j,k,idest), & 
     &                                             i=1,nxb+2*nguard)
             write(*,56) j,(ez_x(ivar,i,j,k), & 
     &                                             i=1,nxb+2*nguard)
56           format(1x,i3,6(2x,f7.4))
             enddo
            endif

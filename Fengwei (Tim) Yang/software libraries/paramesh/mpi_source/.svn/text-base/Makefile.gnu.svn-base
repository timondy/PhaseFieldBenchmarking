...
PMLIB_RPATH = $(paramesh_dir)/mpi_source
...
%.lo:%.c ../Makefile Makefile.gnu
	libtool  --mode=compile --tag=CC $(CC) -c $(CFLAGS) $<

objects := $(sources:.F90=.lo)
cobjects := $(csources:.c=.lo)

vpath %.fh ../headers

libmpi_paramesh.a: $(objects) $(cobjects)
	libtool  --mode=link --tag=FC $(FC) $(FFLAGS) -o libmpi_paramesh.la $^ $(LDFLAGS) -rpath $(PMLIB_RPATH)
	if [ -f $(PMLIB_RPATH)/.libs/libmpi_paramesh.a ]; then ln -sf $(PMLIB_RPATH)/.libs/libmpi_paramesh.a $(paramesh_dir)/libs/ ; fi
	if [ -f $(PMLIB_RPATH)/.libs/libmpi_paramesh.so ]; then ln -sf $(PMLIB_RPATH)/.libs/libmpi_paramesh.so $(paramesh_dir)/libs/ ; fi

#	libtool  --mode=link --tag=FC $(AR) $(ARFLAGS) $@ $^
ifdef MY_CPP
...
.PHONY: clean
clean:
	$(RM) libmpi_paramesh.a *.o *.lo *.i *~ GNUmakefile.include .libs


local_tree_build.o: find_surrblks.o
find_surrblks.o: local_tree.o

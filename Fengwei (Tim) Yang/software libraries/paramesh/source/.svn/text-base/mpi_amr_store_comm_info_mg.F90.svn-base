...
! CEG - THIS IS THE FILE WITH THE MEMORY LEAK!
      ...
      if (first.eq.-1) first=level
      if (first.gt.level) first=0
...
!         print *,'is associated'
         if(associated(to_be_levels(level) % to_be_received_guard_mg))  then
            deallocate(to_be_levels(level) % to_be_received_guard_mg)
!            print *,'deallocated'
         else
!            print *,'no need to deallocate'
         endif
       else
!         print *,'NOT Deallocated - first=',first
       endif
!      print *,'deallocation tests done'
     ...
     &                               stat = istat)
!         print *,'Allocate to_be_received_guard_mg' 
      else
...
!!!
      ...
      if (first.eq.0) then
!       if(allocated(to_be_levels(level) % to_be_sent_guard_mg))  & 
...
!       deallocate(to_be_levels(level) % to_be_received_guard_mg)
!         print *,'deAllocate to_be_received_guard_mg' 
! Moved by CEG to here
      if (first.gt.0) first = first-1
      return

...
!  3. Perform 2/3 V-cycles
  do i_step=1,3
     call amr_multigrid_v_cycle(pid, noprocs, mg_max_level, defect)
        ...
        do k=nguard*k3d+1, nzb+nguard*k3d
           do j=nguard+1, nyb+nguard
              do i=nguard+1, nxb+nguard
                 previt(i, j, k, lb, v) = unk(1+(v-1)*nunkvbles, i, j, k, lb)
              end do
           end do
        end do
     end do
              ...
              do j=nguard+1, nyb+nguard
                 do i=nguard+1, nxb+nguard
                     ...
                     if (abs(omega).gt.1.0e-16) then
                        normPhi = normPhi + (unk(v,i,j,k,lb)-previt(i, j, k, lb, v))*(unk(v,i,j,k,lb)-previt(i, j, k, lb, v))/(omega*omega)

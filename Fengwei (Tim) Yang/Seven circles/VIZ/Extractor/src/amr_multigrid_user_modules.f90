...
!  real, parameter :: defect_tol=1e-10
  ...
  logical, allocatable, dimension (:) :: has_children,has_gchildren       ! CEG : set but probably not used at the minute
...
module time_dep_parameters
  ...
  double precision :: dt=1.e-16
  ! old value for dt
  double precision :: dtold=1.e-16
...
!  double precision, parameter :: simulation_time=100.0
...
!  double precision :: tau0=1.0,D_therm,w0=1.0,lambda=5.0,delta=-0.2,epsilon=0.02,epsilon_tilda,tau_val
!CEG run 24/2/10 modified epsilon up by a lot, delta from 0.2 to 0.6 and Lewis number up from 100 to 500 for 8017
!  double precision :: tau0=1.0,D_therm,w0=1.0,lambda=5.0,delta=-0.6,epsilon=0.02,epsilon_tilda,tau_val
!CEG run 25/2/10 to reproduce James's case 8075
!  double precision :: tau0=1.0,D_therm,w0=1.0,lambda=5.0,delta=-0.6,epsilon=0.02,epsilon_tilda,tau_val
!CEG run 25/2/10 to reproduce James's case 8075
!  double precision :: tau0=1.0,D_therm,w0=1.0,lambda=3.2,delta=-0.65,epsilon=0.05,epsilon_tilda,tau_val
!  double precision :: D_solute,ke=0.3,le=100,mcinf=0.05
!  double precision :: D_solute,ke=0.3,le=100,mcinf=0.05

  integer :: eighthdmn

!  double precision :: tau0=1.0,D_therm,w0=1.0,lambda=2.0,delta=-.6,epsilon=0.02,epsilon_tilda,tau_val
!  double precision :: D_solute,ke=.15,le=40,mcinf=0.1


! New test case run
  double precision :: tau0, D_therm, w0, lambda, delta, epsilon, epsilon_tilda, tau_val
  double precision :: D_solute, ke, le, mcinf

  double precision :: nucleate_x, nucleate_y, nucleate_z, nuc_radius=5.0

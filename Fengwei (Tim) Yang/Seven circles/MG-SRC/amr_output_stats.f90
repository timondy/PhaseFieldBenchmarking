...
subroutine amr_vcycle_summary(pid, noprocs, i_step, defect, old_defect, npts, rms, convrate)
  ...
  double precision, intent(out) :: convrate
  double precision :: Grms(total_vars), outres
  double precision :: dx
  integer :: Gnpts, ierr, i

  dx = bsize(1,lnblocks)/real(nxb)
...
!1  if(pid.eq.0)write(*,*) sqrt( ((dx*dx)/(3.0*3.2*3.2))*(Grms(1)+Grms(2)+Grms(3)) )
